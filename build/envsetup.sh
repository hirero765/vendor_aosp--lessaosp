...
- pixelgerrit:     A Git wrapper that fetches/pushes patch from/to LessAOSP Gerrit Review.
...
- repopick:        Utility to fetch changes from LessAOSP Gerrit.
...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        echo "Found device"
        if (adb shell getprop org.lessaosp.device | grep -q "$CUSTOM_BUILD"); then
...
function githubremote()
{
    ...
    local REMOTE=$(git config --get remote.lessaospos.projectname)

    local PROJECT=$(echo $REMOTE | sed -e "s#platform/#android/#g; s#/#_#g")

    git remote add github https://github.com/LessAOSPOS/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop org.lessaosp.device | grep -q "$CUSTOM_BUILD");
    then
...
function pixelgerrit() {
    ...
    local review=`git config --get remote.lessaospos.review`
    local project=`git config --get remote.lessaospos.projectname`
...
function pixelrebase() {
    ...
    cd $dir
    repo=$(git config --get remote.lessaospos.projectname)
...
function dopush()
{
    ...
    if (adb shell getprop org.lessaosp.device | grep -q "$CUSTOM_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
